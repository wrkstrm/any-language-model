...
public struct GenerationSchema: Sendable, Codable, CustomDebugStringConvertible {
  indirect enum Node: Sendable, Codable {
    ...
    func encode(to encoder: Encoder) throws {
      ...
      case .object(let obj):
        ...
        var propsContainer = container.nestedContainer(
          keyedBy: DynamicCodingKey.self, forKey: .properties)
        ...
        let shouldOmit =
          encoder.userInfo[GenerationSchema.omitAdditionalPropertiesKey] as? Bool ?? false
        if !shouldOmit {
    ...
    init(from decoder: Decoder) throws {
      ...
      case "object":
        ...
        self = .object(
          ObjectNode(description: description, properties: properties, required: required))
      ...
      case "array":
        ...
        self = .array(
          ArrayNode(description: description, items: items, minItems: minItems, maxItems: maxItems))
  ...
  private static func convertDynamic(
    ...
    case .object(let name, let desc, let properties):
      ...
      for prop in properties {
        props[prop.name] = try convertDynamic(
          prop.schema, nameMap: nameMap, defs: &defs, dynamicProp: prop)
        if !prop.isOptional {
    ...
    case .array(let item, let min, let max):
      ...
      return .array(
        ArrayNode(
          description: dynamicProp?.description, items: itemNode, minItems: min, maxItems: max)
      )
    ...
    case .scalar(let scalar):
      ...
      case .string:
        return .string(
          StringNode(description: dynamicProp?.description, pattern: nil, enumChoices: nil))
      case .number:
        return .number(
          NumberNode(
            description: dynamicProp?.description, minimum: nil, maximum: nil, integerOnly: false)
        )
      case .integer:
        return .number(
          NumberNode(
            description: dynamicProp?.description, minimum: nil, maximum: nil, integerOnly: true)
        )
      case .decimal:
        return .number(
          NumberNode(
  ...
  private static func validateRefs(
    _ node: Node, defs: [String: Node], undefinedRefs: inout [String]
  ) throws {
    switch node {
    case .ref(let name):
      if defs[name] == nil {
  ...
  public init(from decoder: any Decoder) throws {
    ...
    if container.contains(.defs) {
      let defsContainer = try container.nestedContainer(
        keyedBy: DynamicCodingKey.self, forKey: .defs)
...
extension GenerationSchema {
  ...
  static let omitAdditionalPropertiesKey = CodingUserInfoKey(
    rawValue: "GenerationSchema.omitAdditionalProperties")!
}
